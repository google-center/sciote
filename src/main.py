...
def train(chat_file, amount, quotient):
    ...
    while test_right_bound <= int(len(metrics)):
        m_tst_data, w_tst_data, tst_lbls = metrics[test_left_bound:test_right_bound], words[
                                                                                      test_left_bound:test_right_bound], f_lbls[
                                                                                                                         test_left_bound:test_right_bound]
        m_trn_data, w_trn_data, trn_lbls = np.concatenate((metrics[:test_left_bound], metrics[test_right_bound:])), \
                                           np.concatenate((words[:test_left_bound], words[test_right_bound:])), \
                                           np.concatenate((f_lbls[:test_left_bound], f_lbls[test_right_bound:]))
